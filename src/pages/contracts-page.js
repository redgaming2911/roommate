...
function renderList() {
  ...
  list.innerHTML = `<div class="contract-row contract-row-header"><div>Mã</div><div>Phòng</div><div>Đại diện</div><div>Thời hạn</div><div>Giá thuê</div><div>Trạng thái</div><div>Thao tác</div></div>${rows.map((contract) => `<div class="contract-row" data-testid="contract-row">
    <div><strong>${escapeHtml(contract.code)}</strong></div><div>${escapeHtml(rooms.get(contract.roomId)?.code || contract.roomId)}</div><div>${escapeHtml(tenants.get(contract.tenantId)?.name || '—')}</div><div>${formatDate(contract.startDate)} – ${formatDate(contract.endDate)}</div><div>${formatCurrency(contract.rentAmount)}</div><div><span class="contract-status contract-status-${contract.status}">${escapeHtml(CONTRACT_STATUS_LABEL[contract.status] || contract.status)}</span></div>
    <div class="actions"><button data-action="view" data-id="${contract.id}">Xem</button><button data-action="edit" data-id="${contract.id}">Sửa</button>${[CONTRACT_STATUS.DRAFT, CONTRACT_STATUS.PENDING].includes(contract.status) ? `<button data-action="activate" data-id="${contract.id}">Kích hoạt</button>` : ''}${[CONTRACT_STATUS.ACTIVE, CONTRACT_STATUS.SOON_EXPIRE].includes(contract.status) ? `<button data-action="end" data-id="${contract.id}">Kết thúc</button>` : ''}${![CONTRACT_STATUS.ENDED, CONTRACT_STATUS.CANCELLED].includes(contract.status) ? `<button data-action="cancel" data-id="${contract.id}">Hủy</button>` : ''}</div>
  </div>`).join('')}`;
}
